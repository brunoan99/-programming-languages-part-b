...
The goal is usually for a PL types system to be sound (so you can rely on it) but not complete
...
A few function ML rejects even though they do not divide by a string
...
- Almost anything you might like to check statically is undecidable:
  - An static checker cannot do all of:
    (1) always terminate
    (2) be sound
    (3) be complete
