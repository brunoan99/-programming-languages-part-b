...
- Common way to define a PL's static checking is via a type systtem
  - Approach is to give each variable, expression, etc. a type
  - Purposes include preventing misuse of primititves (e.g., 4/"hi"),
    enforcing abstraction, and avoiding dynamic checking
      - Dynamic means at run-time

- Dynamically-typed languages do (almost) do static checking
...
Have discussed facts about what the ML type system does and does not prevent
  - Separeate from how (e.g., one type for each variable) though previously studied many of ML's typing rules
